...
export const uploadDocuments = internalAction({
  ...
  handler: async (ctx, { documentIds }) => {
    ...
    await map(documentIds, async (documentId) => {
      ...
      await (openai.beta.assistants as any).files.create(process.env.ASSISTANT_ID!, {
        file_id: fileId,
      });
...
export const getDocument = internalQuery(async (ctx, { documentId }: { documentId: Id<'documents'> }) => {
  return await ctx.db.get(documentId);
});

export const saveFileId = internalMutation(async (ctx, { documentId, fileId }: { documentId: Id<'documents'>; fileId: string }) => {
